...
def environment():
    ...
    storage.append((ps[0].value)/NORMALIZATION_SIZE)
    storage.append((ps[1].value)/NORMALIZATION_SIZE)
    storage.append((ps[2].value)/NORMALIZATION_SIZE)
    storage.append((ps[5].value)/NORMALIZATION_SIZE)
    storage.append((ps[6].value)/NORMALIZATION_SIZE)
    storage.append((ps[7].value)/NORMALIZATION_SIZE)
...
def Reward(state,next_state):
    ...
    for i in range(MAX_FRAME):                                          # 각 프레임에 대해 모두 진행
        if next_state[i * input] < ARRIVE_STANDARD:
            total += 10
    total += (state[0] - next_state[0]) * 100
    # print("angle : ",abs(state[1]) - abs(next_state[1]))
    return total
